...
describe("Todos", () => {
      ...
      testApp.get("/todos")
        ...
        .end((err, res) => {
          ...
          for (const index in res.body) {
            if (res.body.hasOwnProperty(index)) {
              const todo = res.body[index];
  ...
  describe("GET /todos/:id", () => {
    ...
    it(
      "responds with todo item when requested with an existing item id",
      (done) => {
  ...
  describe("DELETE /todos/:id", () => {
    ...
    it(
      "responds with deleted todo item when requested with an existing item id",
      (done) => {
        const todoId = createdTodoIds[0];
